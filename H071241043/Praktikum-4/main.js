document.addEventListener("DOMContentLoaded", () => {
  ...
  function createDeck() {
    ...
    for (let color of colors) {
      for (let value of values) {
        ...
        // Jika bukan '0', buat OBJEK BARU untuk duplikatnya
        if (value !== "0") {
          const card2 = {
            color: color.js,
            value,
  ...
  function renderHand(hand, element, owner) {
    ...
    hand.forEach((card) => {
      ...
      // Hanya tampilkan "UNO!" jika Bot sisa 1 kartu DAN
      // timer challenge pemain sudah habis (cpuUnoCalled = true)
      if (owner === "cpu" && hand.length === 1 && cpuUnoCalled) {
  ...
  function playerPlayCard(card) {
    ...
    // --- PERBAIKAN BUG KARTU DUPLIKAT ---
    // Gunakan splice untuk menghapus HANYA 1 kartu
  ...
  function cpuTurn() {
    ...
    setTimeout(() => {
      ...
      if (cardToPlay) {
        // --- PERBAIKAN BUG KARTU DUPLIKAT (untuk Bot) ---
        const cardIndex = cpuHand.indexOf(cardToPlay);
        if (cardIndex > -1) {
        ...
        clearTimeout(challengeTimer);
        challengeUnoButton.classList.add("hidden");
        cpuUnoCalled = false; // Pastikan false
        pendingCpuCard = cardToPlay;

        renderAll(); // Render kartu (tanpa tulisan "UNO!")

        if (cpuHand.length === 1) {
          challengeUnoButton.classList.remove("hidden");
          cpuUnoCalled = false;
          challengeTimer = setTimeout(() => {
            // Waktu habis, Bot "berhasil" bilang UNO
            cpuUnoCalled = true; // Set jadi true
            challengeUnoButton.classList.add("hidden");
            updateStatus("Bot berhasil UNO!");
            
            renderAll(); // Render ulang untuk MEMUNCULKAN tulisan "UNO!"

            if (pendingCpuCard) {
              handleCardEffect(pendingCpuCard, "cpu");
            } else {
              switchTurn(); // Fallback
            }
          }, 3000); // Timer 3 detik
  ...
  function handleCardEffect(card, playedBy) {
    ...
    pendingCpuCard = null; // Bersihkan kartu tertunda
      ...
      case "Draw2":
        ...
        updateStatus(
          `${playedBy.toUpperCase()} main ${
            card.value
          }. ${opponent.toUpperCase()} mengambil 2 kartu.`
        );
        switchTurn();
        return;
  ...
  function startUnoTimer(player) {
    ...
    unoTimer = setTimeout(() => {
      if (!unoCalled) {
        updateStatus(
          `${player.toUpperCase()} lupa UNO! Bot memanggil Anda. Penalti +2 kartu.`
        );
        drawCards(player, 2);
        renderAll();
      }
